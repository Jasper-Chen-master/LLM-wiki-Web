...
app.get("/api/ai-status", (_req, res) => res.json({ provider: process.env.OPENROUTER_API_KEY?.trim() ? "openrouter" : "demo", configured: Boolean(process.env.OPENROUTER_API_KEY?.trim()) }));
